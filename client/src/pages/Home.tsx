  ...
  return <div className="min-h-screen bg-[#f6f2ec] text-[#19221c]">
    ...
    <main className="lg:ml-[244px]">
      <div className="mx-auto max-w-[1480px] px-4 py-6 sm:px-7 sm:py-8 lg:px-10 lg:py-10">
        ...
        {view === "nutrition" && <NutritionView profile={profile} />}
...
function NutritionView({ profile }: { profile: Profile }) {
  const fuel = nutrition(profile);
  return <div className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]"><section className="paper-card overflow-hidden"><div className="relative h-56"><img src={ASSETS.nutrition} alt="Balanced cricket training meal" className="h-full w-full object-cover" onError={imageFallback} /><div className="absolute inset-0 bg-gradient-to-r from-[#17251c]/80 via-[#17251c]/20 to-transparent" /><div className="absolute bottom-6 left-6 text-[#f5efe8]"><div className="eyebrow text-[#c7d2c6]">Your fuel plan</div><h2 className="mt-2 font-display text-3xl font-semibold tracking-[-0.05em]">Built for your week.</h2></div></div><div className="p-5 sm:p-7"><div className="grid gap-3 sm:grid-cols-4">{[{ label: "kcal / day", value: fuel.calories }, { label: "protein", value: `${fuel.protein}g` }, { label: "carbs", value: `${fuel.carbs}g` }, { label: "fats", value: `${fuel.fats}g` }].map((metric) => <div className="nutrition-metric" key={metric.label}><div className="font-display text-2xl font-semibold">{metric.value}</div><div className="nutrition-label">{metric.label}</div></div>)}</div><div className="mt-7"><div className="eyebrow mb-3">Simple daily rhythm</div><div className="space-y-3">{fuel.meals.map((meal, index) => <div key={meal} className="meal-row"><span className="meal-number">0{index + 1}</span><span className="text-sm font-bold text-[#4b554d]">{meal}</span></div>)}</div></div></div></section><aside className="space-y-5"><section className="coach-note-card p-5 sm:p-6"><div className="eyebrow mb-2 text-[#b46a44]">Why these numbers?</div><h3 className="font-display text-2xl font-semibold leading-tight tracking-[-0.04em]">Enough fuel to train. Enough clarity to adjust.</h3><p className="mt-3 text-sm leading-6 text-[#765d4f]">CoachIQ uses your body weight, training frequency, role, and level for a general starting estimate rather than a rigid rule.</p></section><section className="surface-card p-5 sm:p-6"><div className="flex items-center gap-3"><IconBadge tone="blue"><Utensils size={17} /></IconBadge><div><div className="eyebrow">Preference on file</div><div className="mt-1 text-sm font-bold">{profile.diet}</div></div></div><p className="mt-4 text-xs leading-5 text-[#938a81]">Update your profile whenever food preferences or training load change.</p></section></aside></div>;
}
